...
from WahabX.utils.database import get_lang, is_maintenance
from WahabX.utils.premium import normalize_text
...
for filename in os.listdir(r"./strings/langs/"):
    if filename.endswith(".yml") and filename != "en.yml":
        ...
        try:
            ...
            print(
                "There is an issue with the language file. Please report it to WahabChatz at @Wahab_Chatz on Telegram"
            )
            sys.exit()
...
if not commands:
    print(
        "There's a problem loading the command files. Please report it to WahabChatz at @Wahab_Chatz on Telegram"
    )
    sys.exit()
